...
class TurnazioneVVF:
	...
	def __init__(self, args):
		...
		if args.media_notti[-1] == "+":
			...
			print("ERRORE: specificare se le notti medie sono un minimo ({}+) o massimo ({}-)".format(num_medio_notti, num_medio_notti))
			exit(-1)
		...
		for vigile in self.vigili:
			if self.DB[vigile].esenteServizi():
				continue
			...
			if num_medio_notti > 0 and not self.DB[vigile].esenteNotti():
				...
				if self.DB[vigile].notti_non_standard and notti_attese >= num_medio_notti:
					print("\t{} avrà {} notti, più della media ~{}.".format(str(self.DB[vigile]), notti_attese, num_medio_notti))
					self.constr_notti_non_standard[vigile] = self.solver.Constraint(notti_attese, notti_attese, "constr_notti_non_standard({})".format(vigile))
					...
					print("\t{} avrà {} notti, meno della media ~{}.".format(str(self.DB[vigile]), notti_attese, num_medio_notti))
					self.constr_notti_non_standard[vigile] = self.solver.Constraint(notti_attese, notti_attese, "constr_notti_non_standard({})".format(vigile))
					for notte in self.var_notti.keys():
						if vigile in self.var_notti[notte].keys():
							self.constr_notti_non_standard[vigile].SetCoefficient(self.var_notti[notte][vigile], 1)
					...
					if notti_medie_da_sotto:
						self.constr_notti_non_standard[vigile] = self.solver.Constraint(notti_attese, notti_attese+1, "constr_notti_non_standard({})".format(vigile))
					else:
						self.constr_notti_non_standard[vigile] = self.solver.Constraint(notti_attese-1, notti_attese, "constr_notti_non_standard({})".format(vigile))
					for notte in self.var_notti.keys():
						if vigile in self.var_notti[notte].keys():
							self.constr_notti_non_standard[vigile].SetCoefficient(self.var_notti[notte][vigile], 1)
			...
			if not self.DB[vigile].esenteSabati():
				...
				if sabati_extra > 0:
					self.constr_sabati_vigile[vigile] = self.solver.Constraint(_NUM_MAX_SABATI+sabati_extra, _NUM_MAX_SABATI+sabati_extra, "constr_sabati_vigile({})".format(vigile))
				else:
					self.constr_sabati_vigile[vigile] = self.solver.Constraint(_NUM_MIN_SABATI, _NUM_MAX_SABATI, "constr_sabati_vigile({})".format(vigile))
			...
			if not servizi_compleanno:
				...
				if compleanno in self.var_notti.keys() and not (self.DB[vigile].esenteNotti()):
				...
				if compleanno in self.var_sabati.keys() and not (self.DB[vigile].esenteSabati()):
					self.constr_compleanno_vigile[vigile].SetCoefficient(self.var_sabati[compleanno][vigile], 1)
				if compleanno in self.var_festivi_vigile.keys()  and not (self.DB[vigile].esenteFestivi()):
			...
			if not self.DB[vigile].esenteServizi():
				...
				for giorno in range(len(self.var_notti.keys())):
					...
					if vigile in self.var_notti[giorno].keys():
						if not (self.giorno_squadra[giorno] in self.DB[vigile].squadre or 0 in self.DB[vigile].squadre):
							...
							if self.DB[vigile].extraNotti() > 0 or "NottiAncheFuoriSettimana" in self.DB[vigile].eccezioni:
					...
					if giorno in self.var_festivi_vigile.keys() and gruppo != 0:
						if vigile in self.var_festivi_vigile[giorno].keys():
							self.constr_cost_servizi_vigile[vigile].SetCoefficient(self.var_festivi_vigile[giorno][vigile], 1.5 * mul_compleanno * mul_festivi_onerosi) # Base 1.5 per incoraggiare massima equità
	...
	def PrintSolution(self):
		...
		if self.STATUS == pywraplp.Solver.INFEASIBLE:
			...
			for vigile in self.vigili:
				...
				line += ": {}".format(self.DB[vigile].notti+self.DB[vigile].sabati+ self.DB[vigile].festivi)
				line += "\n\tNotti: {}".format(self.DB[vigile].notti)
				line += "\n\tSabati: {}".format(self.DB[vigile].sabati)
				line += "\n\tFestivi: {}".format(self.DB[vigile].festivi)
				if len(self.DB[vigile].eccezioni) > 0:
		...
		if self.STATUS == pywraplp.Solver.INFEASIBLE:
			...
			for giorno in range(len(self.var_notti.keys())):
				...
				if giorno in self.var_sabati.keys():
					frag = ""
					for vigile in self.vigili:
						if not self.DB[vigile].esenteSabati() and self.var_sabati[giorno][vigile].solution_value() == 1:
			...
			for vigile in self.vigili:
				if (
					...
					and not self.DB[vigile].esenteServizi()
			...
			print("Media servizi notturni per vigile senza vincoli aggiuntivi ({}): {}".format(i, media_notti))
